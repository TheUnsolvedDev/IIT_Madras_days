...
class Algorithm:
    ...
    def simulate(self):
        ...
        for i in range(self.max_iteration):
            ...
            theta, prod = self.solver.solve_LOG_kernel(self.A, self.b, 0.01)
            theta += jax.random.normal(self.env.rng, (self.env.size,))*0.01
            theta = jnp.clip(theta, 0, 1)

            if i % 1 == 0:
                print('action taken:', idx_to_coord(action, self.env.size)[
                      0], idx_to_coord(action, self.env.size)[1],'rank:',jnp.linalg.matrix_rank(jnp.array(self.A)))
                hmap = np.vstack([mask, theta, self.theta_star])
                plt.imshow(hmap)
                plt.show()

            eigen_values, eigen_vectors = jnp.linalg.eigh(prod)
            action = jnp.argmax(self.all_mask@eigen_vectors[0])
