...
class PolicyNetwork(flax.linen.Module):
    def apply(self, x, n_actions):
        dense_layer_1 = flax.linen.Dense(x, 64)
        activation_layer_1 = flax.linen.relu(dense_layer_1)
        dense_layer_2 = flax.linen.Dense(activation_layer_1, 32)
        activation_layer_2 = flax.linen.relu(dense_layer_2)
        output_dense_layer = flax.linen.Dense(activation_layer_2, n_actions)
        output_layer = flax.linen.softmax(output_dense_layer)
        return output_layer
...
policy_network = flax.linen.Model(pg_module, params)
...
def train_step(optimizer, batch):
    def loss_fn(model):
        ...
        picked_action_probabilities = gather(
            action_probabilities_list, batch[1])
...
try:
    for episode in range(num_episodes):
        ...
        while True:
            ...
            action_probabilities = policy_inference(
                optimizer.target, jnp.asarray([state]))[0]
            ...
            if done:
                ...
                # https://twitter.com/araffin2/status/1329382226421837825
                discounted_rewards = discounted_rewards / \
